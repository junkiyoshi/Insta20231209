...
void ofApp::update() {
	...
	for (float threshold_base = 0.4; threshold_base < 0.7; threshold_base += 0.2) {

		float threshold_1 = threshold_base - 0.065;
		float threshold_2 = threshold_base + 0.065;
		for (int i = 0; i <= 6; i++) {
			...
			for (float x = -len; x <= len; x += span) {
				...
				for (float y = -len; y <= len; y += span) {
					...
					ofColor face_color;
					face_color.setHsb(ofMap(threshold_base, 0.4, 0.7, 0, 255), 180, 255);
